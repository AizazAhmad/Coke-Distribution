                                ...
                                <div class="col-sm">
                                    <form id="myform">
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-sm-10">
                                                ...
                                                <select class="form-control" id="coolerjson" name="CoolerId">
                                                    <?php for ($i=0; $i < count($cooler); $i++) {
                                                         ?>
                                                    <option value="<?=$cooler[$i]->Id?>"><?=$cooler[$i]->Code?></option>                                        <?php } ?>            
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group row mb-0">
                                            <div class="col-sm-6">
                                                <button type="submit" id="submit" class="btn btn-gradient-info">Assign</button> 
                                            </div>
            ...
            $.ajax({
                 ...
                 success: function(data){
                  ...
                  $.each(data, function(key, value){
                      bookerlist += '<option value=' + value.Id + '>' + value.Code + ' ' + value.Name + '</option>';
                  });
            $("#customerjson").html(bookerlist);
    ...
    transitionOut: 'flipOutX'
      ...
      $("#myform").submit(function (e) {
        ...
        $.ajax({
          ...
          complete: function() {
                $('#submit').empty().addClass('btn btn-gradient-info').html('Assigned');
          ...
          success: function (response) {
            if (response == "Success") {
              ...
              $("#customerjson option:selected").remove();
              $("#coolerjson option:selected").remove();
