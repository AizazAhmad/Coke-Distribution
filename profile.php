...
$Title = "Profile";
$ProfileQuery = "SELECT dm.Date,dm.Code,dm.Name,dm.Mobile,dm.Status,er.Name AS Role FROM deliveryman dm INNER JOIN emprole er ON dm.RoleId = er.Id WHERE dm.Id = {$_POST['id']} AND dm.UserId = {$_SESSION['user_id']}";

$Result = $db->query($ProfileQuery);
$rows = $Result->fetch_object();

$ExpenseQuery = "SELECT em.Date,em.Amount,em.Description,t.Name AS Type FROM empexpense em INNER JOIN empexpmaintype t ON em.EMTypeId = t.Id WHERE YEAR(Date) = YEAR(CURRENT_DATE()) AND 
      MONTH(Date) = MONTH(CURRENT_DATE()) AND EmpId = {$_POST['id']} AND em.UserId = {$_SESSION['user_id']}";
...
$Satitics = "SELECT  (                
                    SELECT SUM(Amount) FROM empexpense WHERE MONTH(Date) = MONTH(CURRENT_DATE())
AND YEAR(Date) = YEAR(CURRENT_DATE()) AND EmpId = {$_POST['id']} AND UserId = {$_SESSION['user_id']}
                    ) AS Expenses,
                    (
                  ...
                  <div class="col-lg-6">
                    <div class="media align-items-center">
                      ...
                      <div class="media-body">
                        <div class="text-white text-capitalize display-6 mb-5 font-weight-400"><?=$rows->Name?></div>
                      ...
                      <div class="card card-profile-feed">
                        <div class="card-header card-header-action">
                          <div class="media align-items-center">
                            ...
                            <div class="media-body">
                              <div class="text-capitalize font-weight-500 text-dark"><?=$rows->Name?></div>
                              <div class="font-13"><?=$rows->Role?></div>
                            </div>
                          </div>
                          <!-- <div class="d-flex align-items-center card-action-wrap">
                        ...
                        <div class="row text-center">
                          ...
                          <div class="col-6 pl-0">
                            <div class="pa-15">
                              <span class="d-block display-6 text-dark mb-5"><?php 
                              if ($rows->Role == "DeliveryMan")
                               echo round($strows->Sales,0);
                              else
                               echo "None";
